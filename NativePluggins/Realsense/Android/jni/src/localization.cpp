...
int findBestMatchingKeyframe(cv::Mat descriptors1, std::vector<cv::KeyPoint> kp1Filtered, std::vector<cv::Point2f>& pts1, std::vector<cv::Point2f>& pts2) {
    ...
    for (int keyframeIndex = 0; keyframeIndex < keyframeCount; keyframeIndex++) {
        ...
        // const auto& currentKeyframe = *container.getKeyframe(keyframeIndex);
...
void translationCalc(std::vector<cv::Point2f>& pts1, std::vector<cv::Point2f>& pts2, cv::Mat& global_tvec, cv::Mat& global_R, rs2::frame colorFrame, rs2::depth_frame depth) {
        ...
        if (pts1.size() >= 2 && pts2.size() >= 2) {
            for (size_t i = 0; i < pts1.size(); ++i) {
                ...
                if (vPixeldepth[0] >= 0 && vPixeldepth[0] < width &&
                    ...
                    // vPixeldepth is within the valid range, so you can call get_distance
                    float vDepth = depth.get_distance(vPixeldepth[0], vPixeldepth[1]);
                    // std::cout << "Depth at (" << vPixeldepth[0] << ", " << vPixeldepth[1] << "): " << vDepth << std::endl;
                    float vPoint[3];
