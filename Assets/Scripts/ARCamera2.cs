...
public class ARCamera2 : MonoBehaviour
{
    /// <summary>
    ...
    bool ChangedSign(float a, float b)
    {
        return (a < 0 && b > 0) || (a > 0 && b < 0);
    }

    // <summary>
    // Restore the rotation if any of the rotation delta axes changes from 
    // negative to positive (or vice-versa).
    // </summary>
    void RestoreRotation()
    {
        Vector3 avg1 = _lastAverageRotationDelta;
        Vector3 avg2 = _averageRotationDelta;
        if (ChangedSign(avg1.x, avg2.x) || ChangedSign(avg1.y, avg2.y) || ChangedSign(avg1.z, avg2.z))
        {
